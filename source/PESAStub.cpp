...
namespace Algorithm
{
	namespace Stubs
	{
		...
		GaPESAStub::GaPESAStub(int populationID,
			int statisticsID,
		...
		void GaPESAStub::SetPopulationParameters(Population::GaPopulationParams params)
		{
		...
		void GaPESAStub::SetFitnessOperation(const Population::GaPopulationFitnessOperationSetup& fitnessOperation)
		{
		...
		void GaPESAStub::SetMating(const Chromosome::GaMatingSetup& mating)
		{
			_mating = mating;
		...
		void GaPESAStub::SetSelectionParameters(Population::SelectionOperations::GaTournamentSelectionParams params)
		{
		...
		void GaPESAStub::SetCoupling(const Population::GaCouplingSetup& coupling)
		{
		...
		// Sets PESA parameters
		void GaPESAStub::SetPESAParameters(const Multiobjective::PESA::GaPESAParams& params)
		{
			_pesaParameters = params;

			// update operation setup stored in flow step if stub is connected
			if( IsConnected() )
			{
				Population::GaScalingSetup setup = _scalingStep->GetSetup();
				setup.SetParameters( &_pesaParameters );
				_scalingStep->SetSetup( setup );
			}
		}

		// Sets grid that splits fitness hyperscape
		void GaPESAStub::SetGrid(const Multiobjective::GaFitnessHyperGrid& grid)
		{
			_grid = grid.Clone();

			// update PESA operation setup stored in flow step if stub is connected
			if( IsConnected() )
			{
				Population::GaScalingSetup setup = _scalingStep->GetSetup();
				( (Multiobjective::PESA::GaPESAConfig&)setup.GetConfiguration() ).SetGrid( *_grid );
				_scalingStep->SetSetup( setup );
			}
		}

		// Connects stub to the workflow
		void GaPESAStub::Connected()
		{
			...
			_scalingStep = new Population::GaScalingStep(
				Population::GaScalingSetup( &_pesaOperation, &_pesaParameters, &Multiobjective::PESA::GaPESAConfig( *_grid, &Fitness::Representation::GaMVFitnessParams( 2 ) ) ),
